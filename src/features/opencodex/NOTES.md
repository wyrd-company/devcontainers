...
`ocx update` works inside the container as the service user. The system `ocx` wrapper serializes updates and holds the supervised proxy down while the package is replaced, then s6 restarts it on the new version.
