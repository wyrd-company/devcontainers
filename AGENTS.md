...
- Secret delivery: the OpenBao Agent feature renders `/run/openbao/secrets/<feature-id>.env` from a user-written `template`, group-readable by `openbao-secrets`. A service feature that consumes secrets adds its service user to that group and an s6 dependency on `openbao-secrets` when they exist, waits for its file at start, and loads literal `NAME=value` lines. A malformed line stops the service. The container environment takes precedence over the file. Without the Agent feature, nothing changes.
